...
import {
  ArrowLeft, CheckSquare, Loader2, Plus, BarChart3,
  Filter, Search, X, CheckCircle, AlertTriangle,
} from "lucide-react"
...
export default function TripChecklistPage() {
  ...
  const fetchTasks = async (retry = false) => {
    ...
    try {
      // Don't show loading for updates, only for initial load
      ...
      if (!res.ok && (res.status === 401 || res.status === 403) && !retry) {
        if (await refreshToken()) return fetchTasks(true)
      }
      ...
      if (res.ok) {
        ...
        setTasks([...newTasks]) // Create completely new array reference
        
        // Force component re-render
        setRefreshKey(prev => prev + 1)
        
        console.log(`[CHECKLIST] Refreshed ${newTasks.length} tasks`, 
          newTasks.map(t => ({ id: t.id, completed: t.is_completed })))
      }
    } catch (error) {
  ...
  /* ───── add task ───── */
  const addTask = async (retry = false) => {
    if (!tripId || !taskForm.title.trim()) return

    try {
      setSaving(true)
      
      // Prepare the request body - Fixed date handling for timezone issues
      ...
      // Only add due_date if it's provided and valid - Send as naive datetime
      if (taskForm.due_date && taskForm.due_date.trim()) {
        try {
          // Create a naive datetime string (without timezone info)
          const dateObj = new Date(taskForm.due_date)
          // Format as YYYY-MM-DD HH:MM:SS (naive format)
          const year = dateObj.getFullYear()
          const month = String(dateObj.getMonth() + 1).padStart(2, '0')
          const day = String(dateObj.getDate()).padStart(2, '0')
          
          // Send as naive date string - backend will handle as local time
          requestBody.due_date = `${year}-${month}-${day}T00:00:00`
          
        } catch (dateError) {
          console.error("[ADD-TASK] Invalid date format:", taskForm.due_date)
      ...
      console.log("[ADD-TASK] Request body:", requestBody)
      ...
      if (!res.ok && (res.status === 401 || res.status === 403) && !retry) {
        if (await refreshToken()) return addTask(true)
      }
      ...
      if (res.ok) {
        ...
        // Force refresh with delay to ensure backend consistency
        setTimeout(async () => {
          await fetchTasks()
        }, 200)
      } else {
        // ✅ ENHANCED ERROR HANDLING - Show backend error in modal
        try {
        ...
        setShowError(true)
        console.error("[ADD-TASK] Failed to add task:", res.status)
      }
    } catch (error) {
  ...
  /* ───── Enhanced task updated callback - HANDLES BOTH SUCCESS AND ERROR ───── */
  const handleTaskUpdate = async (message: string, taskId?: number, isError: boolean = false) => {
    console.log("[CHECKLIST] Task update:", { message, taskId, isError })
    
    if (isError) {
      // ✅ SHOW ERROR MODAL
      setErrorTitle("Task Operation Failed")
      ...
      // Refresh from server for successful operations
      await fetchTasks()
...
return (
  <DashboardShell>
    <div className="container mx-auto px-6 py-8">
      ...
      {/* Summary - Moved outside header */}
      {tasks.length > 0 && (
        <div className="mb-6 grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="text-center p-4 bg-muted/20 rounded-lg">
            <div className="text-2xl font-bold text-[#1e40af]">{tasks.length}</div>
            <div className="text-sm text-muted-foreground">Total Tasks</div>
          </div>
          <div className="text-center p-4 bg-muted/20 rounded-lg">
            <div className="text-2xl font-bold text-green-600">{tasks.filter(t => t.is_completed).length}</div>
            <div className="text-sm text-muted-foreground">Completed</div>
          </div>
          <div className="text-center p-4 bg-muted/20 rounded-lg">
            <div className="text-2xl font-bold text-orange-600">{tasks.filter(t => !t.is_completed).length}</div>
            <div className="text-sm text-muted-foreground">Pending</div>
          </div>
          <div className="text-center p-4 bg-muted/20 rounded-lg">
            <div className="text-2xl font-bold text-red-600">{tasks.filter(t => t.priority === TaskPriority.URGENT).length}</div>
      ...
      {showError && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
          <div className="bg-background border border-border/50 rounded-xl p-6 max-w-md w-full shadow-2xl">
            ...
            <h3 className="font-semibold text-lg mb-2 text-center text-red-800 dark:text-red-300">{errorTitle}</h3>
