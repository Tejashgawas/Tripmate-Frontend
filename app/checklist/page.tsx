...
export default function ChecklistPage() {
  ...
  const getCurrentUser = async () => {
    try {
      console.log("[CHECKLIST] Fetching current user from /me endpoint...")
      const response = await fetch(`${BASE_URL}me/`, {
        credentials: "include"
      })
      if (response.ok) {
        const userData = await response.json()
        console.log("[CHECKLIST] Current user data:", userData)
        setCurrentUserId(userData.id)
        return userData.id
      } else {
        console.error("[CHECKLIST] Failed to fetch current user:", response.status)
      }
    } catch (error) {
      console.error("[CHECKLIST] Error fetching current user:", error)
    }
    return null
  }
  ...
  const fetchTrips = async (retry = false) => {
    try {
      ...
      if (!userId) {
        console.error("[CHECKLIST] Cannot fetch trips - no user ID")
        return
      }

      console.log("[CHECKLIST] Fetching trips for user:", userId)
      ...
      if (res.ok) {
        ...
        console.log("[CHECKLIST] Raw API response:", data)
        ...
        console.error("[CHECKLIST] Failed to fetch trips:", res.status)
      }
    } catch (error) {
      ...
      <div className="container mx-auto px-4 sm:px-6 py-6 sm:py-8">
            ...
            <div className="group relative">
              ...
              <Card className="relative p-6 sm:p-8 bg-white/90 dark:bg-gray-900/90 backdrop-blur-xl border border-purple-200/50 dark:border-purple-800/50 rounded-xl sm:rounded-2xl hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
                <div className="flex items-center gap-4 sm:gap-6">
                  ...
                  <div>
                    <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-400 mb-1 sm:mb-2 font-medium">Total Checklists</p>
        ...
        {loading ? (
          <div className="flex flex-col items-center py-16 sm:py-20">
            ...
            <p className="mt-4 sm:mt-6 text-lg sm:text-xl text-muted-foreground">Loading your checklists…</p>
          </div>
        ) : tripMemberships.length === 0 ? (
          /* Empty State */
          <div className="text-center py-16 sm:py-20">
            <div className="relative inline-block mb-6 sm:mb-8">
              <div className="w-24 h-24 sm:w-32 sm:h-32 bg-gradient-to-br from-gray-100 to-gray-200 dark:from-gray-800 dark:to-gray-700 rounded-full flex items-center justify-center">
                <CheckSquare className="h-12 w-12 sm:h-16 sm:w-16 text-muted-foreground"/>
              </div>
        ...
        {tripMemberships.length > 0 && (
          <div className="mt-12 pt-8 border-t border-border/30">
            <div className="text-center">
              <h3 className="text-lg font-semibold mb-4">Ready to organize your next trip?</h3>
