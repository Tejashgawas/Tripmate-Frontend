...
import { BASE_URL, fetchWithRetry, fetchMe } from '@/lib/auth';
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (!meResponse || meResponse.role !== "provider") {
          router.push('/login');
          return;
        }

        const [totalRes, recRes, selRes, countRes] = await Promise.all([
          fetchWithRetry(`${BASE_URL}providers/services/count`, { credentials: 'include' }),
          fetchWithRetry(`${BASE_URL}providers/recommended-services`, { credentials: 'include' }),
          fetchWithRetry(`${BASE_URL}providers/selected-services/by-type`, { credentials: 'include' }),
          fetchWithRetry(`${BASE_URL}providers/services/count-by-type`, { credentials: 'include' }),
        ]);

        const data: Stats = {
          total: (await totalRes.json() as CountRes).total_services,
          rec: await recRes.json() as RecItem[],
          selected: await selRes.json() as SelectedItem[],
          typeCount: await countRes.json() as CountByTypeItem[],
        };

        setStats(data);
      } catch (err) {
        console.error('Error:', err);
        if (err instanceof Error && err.message.includes('fetchMe')) {
          router.push('/login');
          return;
        }
        setError(true);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-indigo-50/20 dark:from-slate-900 dark:via-slate-900/95 dark:to-indigo-950/20">
          ...
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-black text-white mb-2 tracking-tight">
                Your Dashboard
                <Sparkles className="inline-block ml-2 w-6 h-6 text-yellow-300" />
              </h1>
      ...
      <section className="relative">
        <div className="bg-gradient-to-br from-white via-slate-50/50 to-blue-50/30 dark:from-slate-800 dark:via-slate-800/95 dark:to-slate-700/50 rounded-2xl p-8 shadow-xl border border-slate-200/50 dark:border-slate-700/50">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {[
              ...
              <div key={index} className="flex items-start space-x-4 p-4 rounded-xl bg-white/50 dark:bg-slate-800/50 border border-slate-200/30 dark:border-slate-600/30 hover:shadow-lg transition-all duration-300">
...
function MetricCard({ 
  ...
  return (
    <div className="group relative overflow-hidden rounded-2xl bg-white dark:bg-slate-800 p-6 shadow-lg border border-slate-200/50 dark:border-slate-700/50 hover:shadow-2xl transition-all duration-500">
...
function ChartCard({ 
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-lg border border-slate-200/50 dark:border-slate-700/50 hover:shadow-xl transition-all duration-300">
...
function InsightCard({ 
  ...
  return (
    <div className={`${bgColor} rounded-2xl p-6 border border-slate-200/30 dark:border-slate-600/30`}>
...
function LoadingSkeleton() {
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50/30 dark:from-slate-900 dark:to-indigo-950/20 p-6">
...
function ErrorState() {
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 to-red-50/30">
      <div className="text-center p-8 bg-white rounded-2xl shadow-xl border border-red-200/50">
