...
export default function ItinerariesPage() {
  ...
  const getCurrentUser = async () => {
    try {
      console.log("[ITINERARIES] Fetching current user from /me endpoint...")
      const response = await fetch(`${BASE_URL}me/`, {
        credentials: "include"
      })
      if (response.ok) {
        const userData = await response.json()
        console.log("[ITINERARIES] Current user data:", userData)
        setCurrentUserId(userData.id)
        return userData.id
      } else {
        console.error("[ITINERARIES] Failed to fetch current user:", response.status)
      }
    } catch (error) {
      console.error("[ITINERARIES] Error fetching current user:", error)
    }
    return null
  }

  /* ───── fetch trips using CORRECT ENDPOINT ───── */
  const fetchTrips = async (retry = false) => {
    try {
      ...
      if (!userId) {
        console.error("[ITINERARIES] Cannot fetch trips - no user ID")
        return
      }

      console.log("[ITINERARIES] Fetching trips for user:", userId)

      // ✅ CORRECT: Use same endpoint as expense page
      ...
      if (res.ok) {
        ...
        console.log("[ITINERARIES] Raw API response:", data)
        ...
        console.error("[ITINERARIES] Failed to fetch trips:", res.status)
      }
    } catch (error) {
      ...
      <div className="container mx-auto px-4 sm:px-6 py-6 sm:py-8">
            ...
            <div className="group relative">
              ...
              <Card className="relative p-6 sm:p-8 bg-white/90 dark:bg-gray-900/90 backdrop-blur-xl border border-purple-200/50 dark:border-purple-800/50 rounded-xl sm:rounded-2xl hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
                <div className="flex items-center gap-4 sm:gap-6">
                  ...
                  <div>
                    <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-400 mb-1 sm:mb-2 font-medium">Total Itineraries</p>
        ...
        {refreshing && (
          <div className="mb-6 p-4 bg-[#1e40af]/10 border border-[#1e40af]/20 rounded-lg flex items-center">
            <div className="w-4 h-4 border-2 border-blue-300 border-t-blue-600 rounded-full animate-spin mr-3"></div>
        ...
        {loading ? (
          <div className="flex flex-col items-center py-16 sm:py-20">
            ...
            <p className="mt-4 sm:mt-6 text-lg sm:text-xl text-muted-foreground">Loading your itinerary trips…</p>
          </div>
        ) : tripMemberships.length === 0 ? (
          /* Empty State */
                        ...
                        <div className="space-y-2 sm:space-y-3 pt-3 sm:pt-4">
                          ...
                          {isOwner && (
                            <>
                              ...
                              <Button 
                                variant="outline"
                                className="w-full h-10 sm:h-12 rounded-lg sm:rounded-xl border-2 border-purple-500 text-purple-600 hover:bg-purple-50 dark:hover:bg-purple-900/20 font-semibold transition-all text-sm sm:text-base group/ai"
                                onClick={() => handleAIPlanClick(trip)}
                              >
                                <Sparkles className="h-4 w-4 sm:h-5 sm:w-5 mr-2 group-hover/ai:animate-pulse"/>
                                Plan with AI
